...
class Bmc(object):
    ...
    def update_sdr(self):
        cmd = "ipmitool sdr"
        try:
            ipmi_sdr = ssh_and_exec_cmd(cmd,  self.bbu_ip, BBU_USER, BBU_PASSWORD)
...
class MyBbu(object):
    ...
    def __update_service_is_up(self):
        ...
        str_cmd = ""
        for cmd in cmds:
            str_cmd += " ps aux --sort -%cpu | head -15 | grep -E '{0}' | wc -l;".format(cmd)
        result={'l1app': 0, 'du_layer2': 0, 'gnb_cu_pdcp': 0}
        self.service = result
        try:
            res=[]
            bbu_service = ssh_and_exec_cmd(str_cmd,  self.ip, BBU_USER, BBU_PASSWORD)
            if(bbu_service):
                for str_line in bbu_service.split("\n"):
                    status = str_line.isdigit()
                    if(status):
                        res.append(True if int(str_line) > 0 else False)
    ...
    def __update_bler(self):
        ...
        last_bler = {}
        try:
            logs = ssh_and_exec_cmd(cmd,  self.ip, BBU_USER, BBU_PASSWORD)
...
#                    print({"time":ts, "pci":pci, "bler":bler})
    ...
    def start_bbu(self):
        ...
        try:
            bbu_start = ssh_and_exec_cmd(BBU_start_CMD,  self.ip, BBU_USER, BBU_PASSWORD)
    ...
    def restart_bbu(self):
        ...
        try:
            bbu_restart = ssh_and_exec_cmd(BBU_restart_CMD,  self.ip, BBU_USER, BBU_PASSWORD)
            self.restart_time = time.time()
            print(bbu_restart)
            return True
        except:
    ...
    def stop_bbu(self):
        ...
        try:
            bbu_stop = ssh_and_exec_cmd(BBU_stop_CMD,  self.ip, BBU_USER, BBU_PASSWORD)
            print(bbu_stop)
            return True
        except:
...
def restart():
    ...
    return bbu.restart_bbu()

# GET Service
@app.get("/service/status", tags=["BBU"])
def service():
    logger.debug(f"BBU Get service")
    return bbu.get_service()

# GET BBU information
@app.get("/info", tags=["BBU"])
def info():
    logger.debug(f"BBU Get Info")
    return bbu.get()

# GET Alarm
@app.get("/alarm", tags=["BBU"])
def alarm():
    logger.debug(f"BBU Get alarm")
    return bbu.get_alarm()

# GET BMC
@app.get("/bmc", tags=["BBU"])
def bmc():
    logger.debug(f"BBU BMC SDR")
    return bbu.get_bmc()

# GET ru
@app.get("/ru", tags=["BBU"])
def ru():
    logger.debug(f"BBU RUs")
    return bbu.get_ru()

# GET acc
@app.get("/acc", tags=["BBU"])
def acc():
    logger.debug(f"BBU ACC")
    return bbu.get_acc()

# GET sys_info
@app.get("/sys_info", tags=["BBU"])
def sys_info():
    logger.debug(f"BBU SYSTEM")
    return bbu.get_sys_info()

# GET bler
@app.get("/bler", tags=["BBU"])
def bler():
    logger.debug(f"BBU BLER")
    return bbu.get_bler()
